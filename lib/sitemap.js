...
function createSitemapIndex(conf) {
  return new SitemapIndex(conf.urls, conf.targetFolder, conf.hostname, conf.cacheTime, conf.sitemapName, conf.sitemapSize, conf.sitemapId, conf.callback);
}
 ...
 */
function SitemapIndex(urls, targetFolder, hostname, cacheTime, sitemapName, sitemapSize, sitemapId, callback) {
  ...
  self.chunks.forEach( function (chunk, index) {
    ...
    var filename = self.sitemapName + '-' + self.sitemapId++ + '.xml.gz';
    ...
    var stream = self.fs.createWriteStream(targetFolder + '/' + filename + ".temp");
    ...
    var inp = self.fs.createReadStream(targetFolder + '/' + filename + ".temp");
    var out = self.fs.createWriteStream(targetFolder + '/' + filename);
    inp.pipe(gzip).pipe(out);
    self.fs.unlink(targetFolder + '/' + filename + ".temp" )
  ...
  if (sitemapId === undefined) {
    ...
    xml.push('<sitemap>');
    xml.push('<loc>' + hostname + '/' + 'sitemap-static.xml' + '</loc>');
    xml.push('</sitemap>');
    ...
    xml.push('<?xml version="1.0" encoding="UTF-8"?>');
    xml.push('<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">');
    for(i=1; i<=(self.sitemapId - 1); i++){
      xml.push('<sitemap>');
      xml.push('<loc>' + hostname + '/' + 'sitemap-' + i + '.xml.gz' + '</loc>');
      xml.push('<lastmod>' + new Date() + '</lastmod>');
      xml.push('</sitemap>');
    }
    xml.push('<sitemap>');
