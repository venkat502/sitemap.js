...
function SitemapIndex(urls, targetFolder, hostname, cacheTime, sitemapName, sitemapSize, callback) {
  ...
  self.chunks.forEach( function (chunk, index) {
    ...
    var stream = self.fs.createWriteStream(targetFolder + '/' + filename + index);
    ...
    var inp = self.fs.createReadStream(targetFolder + '/' + filename + index);
    var out = self.fs.createWriteStream(targetFolder + '/' + filename);
    inp.pipe(gzip).pipe(out);
    self.fs.unlink(targetFolder + '/' + filename + index )
